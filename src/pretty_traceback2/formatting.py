...
import pretty_traceback2.common as com
...
STYLE_BRIGHT: str = typ.cast(str, colorama.Style.BRIGHT)
STYLE_RESET: str = typ.cast(str, colorama.Style.RESET_ALL)
STYLE_NORMAL: str = typ.cast(str, colorama.Style.NORMAL)

COLOR_RED: str = typ.cast(str, colorama.Fore.RED)
COLOR_CYAN: str = typ.cast(str, colorama.Fore.CYAN)
COLOR_YELLOW: str = typ.cast(str, colorama.Fore.YELLOW)
COLOR_MAGENTA: str = typ.cast(str, colorama.Fore.MAGENTA)

FMT_MODULE: str = COLOR_CYAN + STYLE_NORMAL + "{0}" + STYLE_RESET
FMT_CALL: str = COLOR_YELLOW + STYLE_NORMAL + "{0}" + STYLE_RESET
FMT_LINENO: str = COLOR_MAGENTA + STYLE_NORMAL + "{0}" + STYLE_RESET
FMT_CONTEXT: str = "{0}"

FMT_ERROR_NAME: str = COLOR_RED + STYLE_BRIGHT + "{0}" + STYLE_RESET
FMT_ERROR_MSG: str = STYLE_BRIGHT + "{0}" + STYLE_RESET
...
def _rows_to_lines(rows: typ.List[PaddedRow], color: bool = False) -> typ.Iterable[str]:
    ...
    for alias, short_module, full_module, call, lineno, context in rows:
        ...
        if alias == "<pwd>":
            yield line.replace(STYLE_NORMAL, STYLE_BRIGHT)
        else:
            yield line
